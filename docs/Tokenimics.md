...
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
✅ All addresses are verified. Distribution transactions are confirmed on-chain. Every PNJC holder can check any wallet's balance in real time via PolygonScan. The deployer address holds 0 PNJC.
...
✅ Value to investors: Once locked, liquidity remains on DEX regardless of any individual wallet compromises. Combined with transparent on-chain distribution and deployer address holding 0 tokens, this provides multi-layered protection.
...
Status	⏳ Planned — to be implemented before or at DEX launch
...
⚠️ Important: Team and Founder vesting follows the same global staged release schedule (10% of remaining every 6 months). This ensures long-term alignment with the project. Full unlock takes many years due to exponential decay. There is no accelerated 12-month full unlock.

✅ Value to investors: The team cannot "dump" tokens after listing. Their financial incentive is tied to the project's long-term success. No tokens are unlocked during the first 6 months.
...
Status	🔒 Locked until DAO activation
...
Contract-level	✅ LIVE	No owner(), no mint(), no admin functions
Distribution	✅ COMPLETE	100% supply distributed to 7 public wallets
Deployer balance	✅ ZERO	Deployer address holds 0 PNJC
Liquidity lock	⏳ Planned	Via Unicrypt/PinkSale at DEX launch
Team vesting	⏳ Planned	Separate vesting contract
Multisig treasury	⏳ Planned	Gnosis Safe 3/5
DAO governance	⏳ Planned	Snapshot + on-chain voting
⚠️ Important: All "Planned" mechanisms will be confirmed via on-chain transactions when executed. Until then, protection relies on the contract architecture and transparent distribution.
...
📌 Note: Technically, initial circulation is approximately 10-13% of total supply. The remaining 87-90% is subject to staged release or lock mechanisms.
...
Concentration at deployer	❌ Eliminated	Deployer holds 0 PNJC; distribution complete
...
Distribution (7 wallets)	Off-contract (from initialOwner)	✅ Complete
Deployer balance	0 PNJC	✅ Verified
Liquidity Lock	Off-contract (Unicrypt/PinkSale)	⏳ Planned
Vesting (Team + Founder)	Off-contract	⏳ Planned
Multisig Treasury	Off-contract (Gnosis Safe)	⏳ Planned
Charity Reserve	Off-contract (public wallet)	✅ Active
🏁 13. Conclusion
...
3	No owner (ownership model intentionally excluded) — full decentralization	✅
...
2	Ownership model excluded	10	10	✅
3	0% Tax on transactions	10	10	✅
4	EIP-2612 (Permit)	10	10	✅
5	Burnable mechanism	10	10	✅
6	Liquidity Lock (50%, planned)	10	8	⏳ Planned, not yet executed
7	Vesting Team + Founder (planned)	10	8	⏳ Planned, not yet executed
8	Multisig Treasury (3/5, planned)	10	8	⏳ Planned, not yet executed
9	Transparent charity (5%)	10	10	✅ Wallet active, on-chain transparent
10	Staged market release (10% → +10%)	10	10	✅
11	Public wallets (7 addresses, verified)	10	10	✅ Distribution confirmed on-chain
12	DAO control (10%)	10	8	⏳ Planned, not yet active
...
📌 Note on scoring: The 5-point deduction reflects that key investor protection mechanisms (liquidity lock, vesting, multisig) are currently at the planning stage. Score will reach 150/150 when these mechanisms are implemented and verified on-chain.
