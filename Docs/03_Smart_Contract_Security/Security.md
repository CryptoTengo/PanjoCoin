...
| :--- | :--- | :--- |
| **PanjoCoin (PNJC)** | ✅ Active | ERC-20 token with Burnable and Permit. Ownerless (`owner = address(0)`). No mint function. |
...
| **DAO Governor** | 🧩 Code Ready | Decentralised governance with quorum (4%) and 7-day voting period. |
...
| **ERC20Burnable** | ✅ | Optional user-driven burning via `burn()` and `burnFrom()`. |
...
**Note:** Independent audits are scheduled before the activation of each module.
...
| **GitHub** | Review the source code at [github.com/CryptoTengo/PanjoCoin](https://github.com/CryptoTengo/PanjoCoin). |
| **Local Verification** | Compile the code locally using Solidity 0.8.36 and compare the bytecode with the deployed contract. |
...
Contracts that require administrative functions (MerkleAirdrop, DAO Governor) use OpenZeppelin's Ownable2Step:
...
| **Private Key Compromise** | Unauthorised fund movement | Multisig wallets, hardware security modules, key rotation. |
...
| :--- | :--- | :--- |
| **Unknown Vulnerabilities** | Despite audits, zero-day exploits are possible. | Continuous monitoring, bug bounty program (planned), community scrutiny. |
| **Network-Level Attacks** | Polygon network compromise. | Polygon's security measures (checkpointing to Ethereum). |
| **Economic Attacks** | Flash loan attacks, price manipulation. | 0% tax design, liquidity depth. |
...
PanjoCoin is developing a bug bounty program to incentivise security research. Details will be announced in **Q4 2026**.
...
| Practice | Description |
| :--- | :--- |
| **Verify Address** | Always verify the contract address before interacting: `0x51ba27A6EB41D879B03ed28eD0A5d6a2982B0BcF`. |
...
| **Too Good to Be True** | Be sceptical of unrealistic promises. |
...
| **Verified Code** | ✅ | ✅ | ✅ |
| **Audit Planned** | ✅ | ✅ | ✅ |
| **ERC20Burnable** | ✅ | N/A | N/A |
| **ERC20Permit (EIP-2612)** | ✅ | N/A | N/A |
| **Zero Transfer Tax** | ✅ | N/A | N/A |
