...
| Independent professional audit (CertiK / Hacken) | ⏳ Not yet started — original Q2 2026 target has passed and is being rescheduled; see [Audits](#-audits) |
...
Full detail in [Tokenomics.md](./Docs/Tokenomics.md). This is a target release schedule; actual release depends on deployment of the VestingVault and related contracts (see [Current Project Status](#-current-project-status)).
...
- Additional modules (Locker, VestingVault, TreasuryVault, MerkleAirdrop, DAO Governor) are code‑complete but **not yet deployed to mainnet**; each will be deployed only after its own independent audit (see [Audits](#-audits)).
...
1. Connect your wallet (e.g., MetaMask) to the Polygon PoS network.
2. Go to Uniswap V2 and select the Polygon network.
3. Paste the contract address: `0x51ba27A6EB41D879B03ed28eD0A5d6a2982B0BcF`.
4. Ensure you have a small amount of POL for gas.
5. Swap for PNJC.

⚠️ **A Uniswap V2 liquidity pool for PNJC is not live yet.** Do not trust any third‑party link or "presale" claiming to sell PNJC before an official listing announcement on the channels listed under [Contacts](#-contacts). Always verify the contract address from official sources.
...
Full 24‑month roadmap in the [Whitepaper](./Docs/WhitePaper.md).
...
1. Review the Whitepaper and Tokenomics.
2. Ensure your proposal aligns with the project's stated architecture (fixed supply, ownerless, zero tax).
3. For significant changes, open an Issue first for discussion.
